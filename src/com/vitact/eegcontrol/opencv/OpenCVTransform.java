...
public class OpenCVTransform
{
	                            ...
	                            int height)
	{
		...
		/* Size to be output */
		Mat display_image = get_normalized_display_image(frame,
		                                                 NORMAL_DISPLAY_WIDTH);

		/* Resize to desired size. */
		Mat li_zoom_frame = get_grey_image(frame, new Size(width, height), false,
		                                   has_border, 8);

		/* Claculate Edges of big image */
		@SuppressWarnings("unused")
		Mat edge_frame = get_edges(frame, new Size(display_image.width(),
	...
	private Mat get_edges(Mat image, Size img_size)
	{
		...
		img_edg = new Mat(new Size(img_frame.width(), img_frame.height()),
		                  CvType.CV_8UC1);
	...
	/*                                                                */
	/******************************************************************/
	private Mat get_grey_image(Mat image,
	                           Size img_size,
	                           boolean equalize,
	                           boolean border,
	                           int border_size)
	{
		...
		if (border == true)
		{
			img_frame = new Mat(new Size(img_size.width - border_size,
			                             img_size.height - border_size),
			                    CvType.CV_8UC1);
		}
		else
		{
			img_frame = new Mat(new Size(img_size.width, img_size.height),
			                    CvType.CV_8UC1);
		}
		...
		if (border == true)
		{
			img_border = new Mat(new Size(img_size.width, img_size.height),
			                     CvType.CV_8UC1);

			Core.copyMakeBorder(img_frame, img_border, border_size >> 1,
			                    border_size >> 1, border_size >> 1, border_size >> 1,
			                    Core.BORDER_CONSTANT, new Scalar(WHITE_COLOR));
	...
	private Mat get_square_image(Mat raw_img)
	{
		...
		/* Image */
		rectSplit = new Rect((raw_img.width() - height) >> 1,
		                     (raw_img.height() - height) >> 1, height, height);
	...
	/*************************************************************************/
	private Mat get_normalized_display_image(Mat reference_img,
	                                         int reference_width)
	{
		Mat img;
		...
		for (int i = 0; i < width; i++)
		{
			for (int j = (height - 1); j >= 0; j--)
			{
				...
				if (myByte < ((WHITE_COLOR * 2) / 3))
				{
					...
					data_ret[byte_num] = (byte) ((byte) data_ret[byte_num]
					    | (1 << (bit_num)));
				} // end if
			} // end for
		} // end for
